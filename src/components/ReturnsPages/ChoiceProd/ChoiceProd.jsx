import React from "react";
import { useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";

////// components
import { Table, TableBody, TableCell, Tooltip } from "@mui/material";
import { TableContainer, TableHead } from "@mui/material";
import { TableRow, Paper } from "@mui/material";

/////// fns
import {
  changeStatusInvoiceReturn,
  delProdInInvoiceReturn,
} from "../../../store/reducers/invoiceSlice";
import {
  changeCountOrders,
  getListProdsInInvoice,
} from "../../../store/reducers/mainSlice";

///// helpers
import { myAlert } from "../../../helpers/MyAlert";
import { validNums } from "../../../helpers/validations";

////// icons
import DeleteIcon from "@mui/icons-material/Delete";
import AddIcon from "../../../assets/MyIcons/AddIcon";

////// style
import "./style.scss";

const ChoiceProd = ({ invoiceGuid }) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const { listSendOrders } = useSelector((state) => state.mainSlice);
  const { checkInvoice } = useSelector((state) => state.mainSlice);

  useEffect(() => {
    dispatch(getListProdsInInvoice(invoiceGuid));
  }, []);

  const onChangeCount = (e, item) => {
    const count = e?.target?.value?.replace(",", ".");

    if (validNums(count)) {
      //// валидация на числа
      return;
    }
    dispatch(changeCountOrders({ ...item, count }));
  };

  const incrementCount = (item) => {
    const newCount = (parseFloat(item?.count) || 0) + 1;
    dispatch(changeCountOrders({ ...item, count: newCount.toString() }));
  };

  const decrementCount = (item) => {
    const newCount = Math.max((parseFloat(item?.count) || 0) - 1, 0); // Минимальное значение - 0
    dispatch(changeCountOrders({ ...item, count: newCount.toString() }));
  };

  const delProd = async ({ product_guid, price, count }) => {
    const products = [{ product_guid, count, workshop_price: price }];
    const data = { invoice_guid: invoiceGuid, status: -1, products };
    const res = await dispatch(
      delProdInInvoiceReturn({ ...data, comment: "" })
    ).unwrap();

    if (res?.result == 1) {
      dispatch(getListProdsInInvoice(invoiceGuid));
    }
  };

  const sendReturn = async () => {
    if (listSendOrders?.length == 0) {
      myAlert("Список пустой", "error");
      return;
    }

    const data = { invoice_guid: invoiceGuid, status: 1 };
    // по умолчанию 0, если удаление -1, 1 - потвержден ТА, 2 подтвержден Админом
    const res = await dispatch(changeStatusInvoiceReturn(data)).unwrap();
    if (res?.result == 1) {
      myAlert("Накладная создана");
      navigate(-1);
    }
  };

  return (
    <div className="choiceProdReturn">
      <TableContainer
        component={Paper}
        sx={{ maxHeight: "100%" }}
        className="scroll_table standartTable"
      >
        <Table stickyHeader aria-label="sticky table">
          <TableHead>
            <TableRow>
              <TableCell style={{ width: "8%" }} align="center">
                №
              </TableCell>
              <TableCell style={{ width: "50%" }}>Продукт</TableCell>
              <TableCell align="left" style={{ width: "10%" }}>
                Кг
              </TableCell>
              <TableCell align="left" style={{ width: "22%" }}>
                Цена
              </TableCell>
              <TableCell
                align="left"
                style={{ width: "10%" }}
                className="titleCheckbox"
              >
                *
              </TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {listSendOrders?.map((row, index) => (
              <TableRow key={row?.product_guid}>
                <TableCell
                  component="th"
                  scope="row"
                  align="center"
                  style={{ width: "6%" }}
                  onClick={() => decrementCount(row)}
                >
                  {index + 1}
                </TableCell>
                <TableCell
                  component="th"
                  scope="row"
                  style={{ width: "44%" }}
                  onClick={() => decrementCount(row)}
                >
                  {row?.product_name}
                </TableCell>
                <TableCell
                  align="left"
                  style={{ width: "17%" }}
                  className="counterRow"
                >
                  {row?.count}
                  <div className="countInputContainer">
                    {/* <input
                      type="text"
                      onChange={(e) => onChangeCount(e, row)}
                      name="counts"
                      value={row?.count}
                      maxLength={10}
                      className="counts"
                      readOnly={!checkInvoice}
                    /> */}
                  </div>
                </TableCell>
                <TableCell
                  align="left"
                  style={{ width: "17%" }}
                  onClick={() => incrementCount(row)}
                >
                  {row?.price} сом
                </TableCell>
                <TableCell align="center" style={{ width: "10%" }}>
                  <Tooltip title={"Удалить"} placement="top" disableInteractive>
                    <button
                      className="actionsDel"
                      disabled={!checkInvoice}
                      onClick={() => delProd(row)}
                    >
                      <DeleteIcon
                        sx={{
                          color: "rgba(213, 42, 42, 0.848)",
                          width: 20,
                          height: 20,
                        }}
                      />
                    </button>
                  </Tooltip>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      </TableContainer>
      <button className="createReturn" onClick={sendReturn}>
        <AddIcon width={16} height={16} color={"#fff"} />
        <p>Оформить возврат</p>
      </button>
    </div>
  );
};

export default ChoiceProd;
